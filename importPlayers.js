...
function newImportShowModal() {
  ...
  if (addBtn) {
    ...
    addBtn.textContent = hasPlayers ? 'Update' : 'Add';
  }
...
function newImportRefreshSelectCards() {
  ...
  source
    ...
    .forEach(p => {
      ...
      card.className = "newImport-player-card" + (added ? " player-added" : "");
      ...
      card.innerHTML = `
        <div class="newImport-player-top">
          ...
          <div class="newImport-player-name">${p.displayName}${added ? ' <span style="font-size:0.7rem;color:var(--accent);">✓</span>' : ''}</div>
          ${statusDot}
        </div>
...
function addPlayersBrowseRender(players) {
  ...
  listEl.innerHTML = players.map(p => {
    ...
    return `
      <div class="newImport-player-card${isSelected ? ' player-added' : ''}${isBlocked ? ' player-busy' : ''}"
        ...
        <div class="newImport-player-top">
          ...
          <div class="newImport-player-name">${name}${isSelected ? ' <span style="font-size:0.7rem;color:var(--accent);">✓</span>' : ''}</div>
          ${playerAvailDot(name)}
        </div>
        <div class="newImport-player-actions">
