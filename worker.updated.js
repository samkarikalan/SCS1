/* ============================================================
KariBRRApp â€” Cloudflare Worker
...
SUPABASE_KEY  = eyJâ€¦  (your anon key)
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // â”€â”€ Club management â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function handleSubActivate(request, env) {
  ...
  if (!rows || !rows.length) return json({ valid: false, error: 'Invalid key â€” already used or does not exist' });
  const lic = rows[0];
...
async function handleAdminClubs(request, env) {
  ...
  const result = clubs.map(c => ({
    ...
    owner_email:    accountMap[c.created_by]?.email    || 'â€”',
    owner_nickname: accountMap[c.created_by]?.nickname || 'â€”',
  }));
...
function selectRestingAndPlaying(state) {
  ...
  // â”€â”€ Gender balancing for typed courts (MD/LD/XD) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (hasTypedDoubles && numResting > 0) {
    ...
    // Sort resting by restQueue position â€” front = rested longest = pull in first
    ...
    // Fix men shortfall â€” swap in resting men, push out surplus women
    if (menNeeded > playingMen.length) {
      swapIn(menNeeded, playingMen, restingMen, [...playingWomen]);
    }
    // Fix women shortfall â€” swap in resting women, push out surplus men
  ...
  // â”€â”€ Unique Games Mode â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (state.uniqueGamesMode && numResting > 0) {
    ...
    // Check if a game is fully unique â€” all 4 opponent cross-matchups are fresh
    ...
    // Score a game â€” opponent freshness primary, partner freshness secondary
    ...
    // Find best game from full pool â€” prioritise fully unique games
      ...
      for (let j = i + 1; j < fullPool.length - 2; j++) {
        for (let k = j + 1; k < fullPool.length - 1; k++) {
          for (let l = k + 1; l < fullPool.length; l++) {
            ...
            // Skip if all 4 already playing â€” current pool handles them
...
function resetForCompetitive(state) {
  // Do NOT wipe pairPlayedSet/gamesMap/opponentMap â€” they contain valid history
...
// â”€â”€ Helper: best pair from pool using existing scoring â”€â”€
...
// â”€â”€ Helper: best matchup from 2 pairs using opponentMap â”€â”€
...
//  Completely self-contained â€” does NOT share state with
//  randomRound or competitiveRound.
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// â”€â”€ Singles matchup scorer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Picks the best 1v1 from a pool using:
//   1. Opponent freshness â€” have A and B never faced each other? (primary)
//   2. Least total opponent count â€” minimise repeat matchups (secondary)
//   3. Pool order (rest queue position) â€” tiebreaker
...
function typedRound(state) {
  ...
  // â”€â”€ 1. Select who rests and who plays â”€â”€
  let { resting, playing } = selectRestingAndPlaying(state);

  // â”€â”€ 2. Same-gender fallback â€” only if NO singles courts defined â”€â”€
  ...
  // â”€â”€ 3. Gender pools sorted by most-rested first â”€â”€
  ...
  // â”€â”€ 4. Resolve effective court type â”€â”€
  ...
  // â”€â”€ 5. Gender swap: pull resting players to satisfy typed courts â”€â”€
  ...
  // â”€â”€ 5. Best pair picker (fully self-contained) â”€â”€
  ...
  for (const c of courtOrder) {
    ...
    if (fmt === 'singles') {
      // â”€â”€ Singles: 1 player per side â”€â”€
      ...
      // â”€â”€ Men's Doubles: best 4 men from remaining â”€â”€
      const pool  = availMen();
      ...
      // â”€â”€ Ladies' Doubles: best 4 women from remaining â”€â”€
      ...
      // â”€â”€ Mixed Doubles: 1 man + 1 woman per pair â”€â”€
      ...
      // â”€â”€ Free / singles-free: best from all remaining players â”€â”€
      const pool  = availAll();
      if (fmt === 'singles') {
        if (pool.length >= 2) {
          const sm = bestSinglesMatch(pool, opponentMap, allRounds);
    ...
    // â”€â”€ Fallback: typed court failed â†’ use remaining players â”€â”€
    if (!game) {
      const pool = availAll();
      if (fmt === 'singles') {
        if (pool.length >= 2) {
  ...
  // â”€â”€ 8. Any playing player not assigned â†’ extra resting â”€â”€
...
//
//  Priority (in order):
//    1. Opponent freshness  â€” how many of the 4 cross-matchups are brand new
//    2. Partner freshness   â€” how long since each pair played together (pairAge)
//    3. Wait fairness       â€” sum of queue positions of the 4 chosen players
...
function mbmBestGame(pool, waitQueue, state) {
  ...
  // â”€â”€ Pair age: rounds since a & b last played together (higher = fresher) â”€â”€
  function pairAge(a, b) {
    const key = pairKey(a, b);
    let lastRound = -1;
    for (let r = 0; r < allRounds.length; r++) {
  ...
  // â”€â”€ Opponent freshness: count cross-matchup pairs never faced â”€â”€
  ...
  // â”€â”€ Wait weight: lower queue index = waited longer = higher weight â”€â”€
  ...
  // â”€â”€ Score a specific game (4 players, 2 pairs already decided) â”€â”€
  function scoreGame(pair1, pair2) {
    const opp  = oppFreshness(pair1, pair2);        // 0â€“4, higher = better
  ...
  // â”€â”€ Try all C(pool,4) combinations, all 3 pairings each â”€â”€
  // For pool size â‰¤ 12 this is at most C(12,4)=495 combos Ã— 3 pairings = 1485 evals â€” fast.
  let best = null;
...
//
//  Key ideas:
//  1. Free courts adapt to remaining players (all men â†’ MD, all women â†’ LD)
...
function typedBestRound(state) {
  ...
  // Use scored ordering as base â€” sort men and women by pair freshness
  ...
  // Players not assigned â†’ extra resting
...
async function handleGenerateRound(request, env) {
  ...
  // â”€â”€ MBM fast path: find best game from pre-filtered pool â”€â”€
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    let roundFn;
    if (hasDoubleTypedCourts) roundFn = typedBestRound;      // MD/LD/XD â†’ new typed best round
    else if (hasSinglesCourts) roundFn = typedRound;          // singles only â†’ existing typedRound
    else if (useComp) roundFn = competitiveRound;             // competitive â†’ existing
    else roundFn = randomRound;                               // free doubles â†’ existing
